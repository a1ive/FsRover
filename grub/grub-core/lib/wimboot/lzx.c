 ...
 * @ret norm_value	Accumulated value (normalised to 16 bits)
 *
...
static int lzx_accumulate (struct lzx *lzx, unsigned int bits)
{
	...
	{
		src16 = grub_get_unaligned16 (lzx->input.data + lzx->input.offset);
		lzx->input.offset += sizeof (grub_uint16_t);
...
static int lzx_getbytes (struct lzx *lzx, void *data, grub_size_t len)
{
	...
	/* Copy data */
	if (data)
		grub_memcpy (data, (lzx->input.data + lzx->input.offset), len);
 ...
 * @v raw		Raw symbol table for the alphabet
 * @ret rc		Return status code
 */
	...
	huffman_raw_symbol_t *raw)
{
	...
	if ((rc = huffman_alphabet (alphabet, raw, lengths, count)) != 0)
		return rc;
 ...
 * @ret rc		Return status code
 */
static int lzx_pretree (struct lzx *lzx, unsigned int count, grub_uint8_t *lengths)
{
	unsigned int i;
	unsigned int length;
	int dup = 0;
	...
	if ((rc = lzx_raw_alphabet (lzx, LZX_PRETREE_CODES, LZX_PRETREE_BITS,
								lzx->pretree_lengths, &lzx->pretree, lzx->pretree_raw)) != 0)
		return rc;
...
static int lzx_alignoffset_alphabet (struct lzx *lzx)
{
	...
	if ((rc = lzx_raw_alphabet (lzx, LZX_ALIGNOFFSET_CODES, LZX_ALIGNOFFSET_BITS,
								lzx->alignoffset_lengths, &lzx->alignoffset, lzx->alignoffset_raw)) != 0)
		return rc;
...
static int lzx_main_alphabet (struct lzx *lzx)
{
	...
	if ((rc = lzx_pretree (lzx, (LZX_MAIN_CODES - LZX_MAIN_LIT_CODES), lzx->main_lengths.remainder)) != 0)
		return rc;

	/* Generate Huffman alphabet */
	if ((rc = huffman_alphabet (&lzx->main, lzx->main_raw, lzx->main_lengths.literals, LZX_MAIN_CODES)) != 0)
		return rc;
...
static int lzx_length_alphabet (struct lzx *lzx)
{
	...
	/* Generate Huffman alphabet */
	if ((rc = huffman_alphabet (&lzx->length, lzx->length_raw, lzx->length_lengths, LZX_LENGTH_CODES)) != 0)
		return rc;
...
static int lzx_block_header (struct lzx *lzx)
{
	...
	case LZX_BLOCK_UNCOMPRESSED :
		...
		if ((rc = lzx_getbytes (lzx, &lzx->repeated_offset, sizeof (lzx->repeated_offset)))!=0)
			return rc;
		break;
	default:
		return -1;
...
static int lzx_uncompressed (struct lzx *lzx)
{
	...
	/* Copy bytes */
	data = (lzx->output.data ? (lzx->output.data + lzx->output.offset) : NULL);
...
static int lzx_token (struct lzx *lzx)
{
	...
	else
	{
		...
		if ((lzx->block_type == LZX_BLOCK_ALIGNOFFSET) && (offset_bits >= 3))
		{
		...
		match_offset = (lzx_position_base[position_slot] + verbatim_bits + aligned_bits - 2);
	...
	/* Copy data */
	if ((match_offset > lzx->output.offset) || (match_offset == 0))
		return -1;
	if (lzx->output.data)
	{
		copy = &lzx->output.data[lzx->output.offset];
		copy_src = (copy - match_offset);
...
static void lzx_translate_jumps (struct lzx *lzx)
{
	...
	for (offset = 0; offset < (lzx->output.offset - 10); offset++)
	{
		...
		/* Translate jump target (unaligned 32-bit access) */
		grub_memcpy (&target, &lzx->output.data[offset + 1], sizeof (target));
		if (target >= 0)
		{
		...
		grub_memcpy (&lzx->output.data[ offset + 1 ], &target, sizeof (target));
...
grub_ssize_t lzx_decompress (const void *data, grub_size_t len, void *buf)
{
	...
	if (! lzx_position_base[LZX_POSITION_SLOTS - 1])
	{
		...
		{
			lzx_position_base[i] = (lzx_position_base[i - 1] + (1 << lzx_footer_bits (i - 1)));
		}
	}
