 ...
 * unaligned reads via grub_get_unaligned16/grub_memcpy, no void
 * pointer arithmetic.
 *
 */
...
static int lzx_accumulate ( struct lzx *lzx, unsigned int bits ) {
	...
	if ( ( lzx->bits < bits ) &&
		...
		src16 = grub_get_unaligned16 ( lzx->input.data +
					       lzx->input.offset );
...
static int lzx_getbytes ( struct lzx *lzx, void *data, grub_size_t len ) {
	...
	/* Copy data */
	if ( data )
		grub_memcpy ( data, ( lzx->input.data + lzx->input.offset ),
			      len );
...
static int lzx_main_alphabet ( struct lzx *lzx ) {
	...
	/* Generate Huffman alphabet */
	if ( ( rc = huffman_alphabet ( &lzx->main, lzx->main_raw,
				       lzx->main_lengths.literals,
				       LZX_MAIN_CODES ) ) != 0 )
		return rc;
...
static int lzx_length_alphabet ( struct lzx *lzx ) {
	...
	/* Generate Huffman alphabet */
	if ( ( rc = huffman_alphabet ( &lzx->length, lzx->length_raw,
				       lzx->length_lengths,
				       LZX_LENGTH_CODES ) ) != 0 )
		return rc;
...
static void lzx_translate_jumps ( struct lzx *lzx ) {
	...
	for ( offset = 0 ; offset < ( lzx->output.offset - 10 ) ; offset++ ) {
		...
		grub_memcpy ( &target, &lzx->output.data[ offset + 1 ],
			      sizeof ( target ) );
		if ( target >= 0 ) {
		...
		grub_memcpy ( &lzx->output.data[ offset + 1 ], &target,
			      sizeof ( target ) );
