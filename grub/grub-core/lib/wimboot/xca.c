...
grub_ssize_t xca_decompress (const void *data, grub_size_t len, void *buf)
{
	...
	/* Construct symbol lengths.  A single Huffman code covers the whole
	 * stream, however long it is: see the format description in
	 * wimlib src\xpress_decompress.c ("this is the only Huffman code
	 * that is used for the entirety of the XPRESS compressed data").
	 * FIX: this used to be redone every XCA_BLOCK_SIZE bytes of
	 * output, which ate 256 bytes of bitstream as a table that is not
	 * there.  Nothing noticed because a WIM compresses each chunk
	 * separately and its chunks do not exceed XCA_BLOCK_SIZE.
	 */
	lengths = src;
	...
	if ((rc = huffman_alphabet (&xca.alphabet, xca.raw, xca.lengths, XCA_CODES)) != 0)
		return rc;

	/* Initialise state */
	if ((src + 2 * sizeof (grub_uint16_t)) > end)
		return -1;
	accum = xca_get16 (&src);
	accum <<= 16;
	accum |= xca_get16 (&src);
	...
	while (src < end)
	{
		...
		else
		{
			...
			{
				match_offset = ((accum >> (32 - match_offset_bits)) + (1U << match_offset_bits));
			}
			else
			{
				match_offset = 1;
