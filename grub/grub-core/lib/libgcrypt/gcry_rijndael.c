/* rijndael.c - AES (Rijndael) cipher for Rover.
 *
 ...
 * setkey/encrypt/decrypt are provided; cryptodisk.c drives ECB/CBC/XTS
 * on top through crypto.c, so no bulk or hardware paths are needed.
 */
...
/* Maximum number of rounds (256-bit key); from rijndael-alg-fst.h.  */
#define RIJNDAEL_MAXNR 14

/* ==== begin public-domain Rijndael "fst" core (rijndael-alg-fst.c) ==== */
...
/* ==== end public-domain Rijndael "fst" core ==== */

/*
 * GRUB gcry glue: expand the reference key schedules once (separate
 * encrypt/decrypt round keys) and expose block operations through the
 * gcry_cipher_spec interface used by grub-core/lib/crypto.c.
 */
...
typedef struct
{
  ...
  uint32_t dk[4 * (RIJNDAEL_MAXNR + 1)];
} RIJNDAEL_context;

static gcry_err_code_t
rijndael_setkey (void *context, const unsigned char *key, unsigned keylen,
                 struct cipher_bulk_ops *bulk_ops)
{
  ...
  ctx->rounds = rijndaelKeySetupEnc (ctx->ek, (const char *) key, keylen * 8);
  rijndaelKeySetupDec (ctx->dk, (const char *) key, keylen * 8);
  return GPG_ERR_NO_ERROR;
}

static unsigned int
rijndael_encrypt (void *context, unsigned char *out, const unsigned char *in)
{
  RIJNDAEL_context *ctx = context;

  rijndaelEncrypt (ctx->ek, ctx->rounds, (const char *) in, (char *) out);
  return 0;
}

static unsigned int
rijndael_decrypt (void *context, unsigned char *out, const unsigned char *in)
{
  RIJNDAEL_context *ctx = context;

  rijndaelDecrypt (ctx->dk, ctx->rounds, (const char *) in, (char *) out);
  return 0;
}

static const char *aes_aliases[] =
  { "AES128", "AES-128", "RIJNDAEL", "RIJNDAEL128", NULL };
static const char *aes192_aliases[] =
  { "AES-192", "RIJNDAEL192", NULL };
static const char *aes256_aliases[] =
  { "AES-256", "RIJNDAEL256", NULL };
