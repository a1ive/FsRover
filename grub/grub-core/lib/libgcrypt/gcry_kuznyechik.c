...
static void kuznyechik_set_key(const byte* key, kuznyechik_kds* kds)
{
	int i;
...
static void kuznyechik_encrypt_block(byte* out, const byte* in, kuznyechik_kds* kds)
{
...
static void kuznyechik_encrypt_blocks(byte* out, const byte* in, size_t blocks, kuznyechik_kds* kds)
{
	while (blocks)
	{
...
static void kuznyechik_decrypt_block(byte* out, const byte* in, kuznyechik_kds* kds)
{
	uint64 x1 = *(const uint64*)in;
	uint64 x2 = *(((const uint64*)in) + 1);
	uint64 t1, t2;
...
static void kuznyechik_decrypt_blocks(byte* out, const byte* in, size_t blocks, kuznyechik_kds* kds)
{
	while (blocks)
	{
...
kuznyechik_setkey (void *context, const unsigned char *key, unsigned keylen, struct cipher_bulk_ops *bulk_ops)
{
	kuznyechik_kds *ctx = context;

	(void) bulk_ops;

	/* GOST R 34.12-2015 only defines a 256 bit key.  */
	if (keylen != 32)
		return GPG_ERR_INV_KEYLEN;

	kuznyechik_set_key (key, ctx);
	return GPG_ERR_NO_ERROR;
}

static unsigned int
kuznyechik_enc_block (void *context, unsigned char *out, const unsigned char *in)
{
	kuznyechik_encrypt_block (out, in, context);
	return 0;
}

static unsigned int
kuznyechik_dec_block (void *context, unsigned char *out, const unsigned char *in)
{
	kuznyechik_decrypt_block (out, in, context);
	return 0;
}

static const char *kuznyechik_aliases[] = { "GRASSHOPPER", "GOST-R-34.12-2015", NULL };

gcry_cipher_spec_t _gcry_cipher_spec_kuznyechik =
{
	.algo = 0,	/* not a libgcrypt algorithm */
	.name = "KUZNYECHIK",
	.aliases = kuznyechik_aliases,
	.blocksize = 16,
	.keylen = 256,
	.contextsize = sizeof(kuznyechik_kds),
	.setkey = kuznyechik_setkey,
	.encrypt = kuznyechik_enc_block,
	.decrypt = kuznyechik_dec_block
};

GRUB_MOD_INIT(gcry_kuznyechik)
{
	grub_cipher_register(&_gcry_cipher_spec_kuznyechik);
}

GRUB_MOD_FINI(gcry_kuznyechik)
{
	grub_cipher_unregister(&_gcry_cipher_spec_kuznyechik);
}
