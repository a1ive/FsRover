		    ...
		    grub_uint8_t *DK, grub_size_t dkLen)
{
  ...
  for (i = 1; i - 1 < l; i++)
      ...
      for (u = 0; u < c; u++)
	{
	  ...
	  /* Rover: report unlock progress every so often (this loop is
	     the expensive part of a cryptodisk PBKDF2).  */
	  if (grub_cryptodisk_kdf_progress != 0 && (u & 0x3ff) == 0)
	    grub_cryptodisk_kdf_progress ((i - 1) * c + u, l * c,
