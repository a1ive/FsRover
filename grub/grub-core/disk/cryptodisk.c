 ...
 * key protectors (grub/key_protector.h) and gcry hardware acceleration
 * (grub/hwfeatures-gcry.h) are dropped.  The GUI unlocks volumes through
 * grub_cryptodisk_mount_by_name() below, passing the passphrase or
 * keyfile bytes directly, so none of that machinery is needed.
 */
		      ...
		      grub_size_t size, char *buf)
{
  ...
  /* Rover: BitLocker (and the like) own the entire read+decrypt path.  */
  if (dev->read_sectors != NULL)
    return dev->read_sectors (dev, sector, size, buf);
		       ...
		       grub_size_t size, const char *buf)
{
  ...
  /* Rover: backends with a bespoke read path (BitLocker) are read only.  */
  if (dev->read_sectors != NULL)
    return grub_error (GRUB_ERR_IO, "device is read-only");
...
grub_cryptodisk_t
grub_cryptodisk_list_head (void)
{
  return cryptodisk_list;
}
				  ...
				  grub_cryptomount_args_t cargs)
{
  ...
  /* Rover: key protectors are not supported; the GUI always supplies a
     passphrase or keyfile via cargs->key_data.  */
...
/*
 * Rover entry point, replacing the interactive `cryptomount' command:
 * open the disk named SOURCE_NAME, run the cryptodisk backends to detect
 * and unlock it with the supplied key bytes, and on success write the
 * resulting device name ("cryptoN") into OUT_NAME.  For a passphrase the
 * key bytes are the passphrase as typed; for a keyfile they are its raw
 * contents.
 */
grub_err_t
grub_cryptodisk_mount_by_name (const char *source_name,
			       const void *key, grub_size_t key_len,
			       char *out_name, grub_size_t out_size)
{
  ...
  /*
   * A key is mandatory: Rover has no terminal, so the interactive
   * passphrase fallback in grub_cryptodisk_scan_device_real() (which
   * relies on grub_getkey) must never be reached.
   */
  if (key == NULL || key_len == 0)
    return grub_error (GRUB_ERR_BAD_ARGUMENT, "no key provided");
  ...
  cargs.key_data = (grub_uint8_t *) key;
  cargs.key_len = key_len;

  dev = grub_cryptodisk_scan_device_real (source_name, disk, &cargs);

  grub_disk_close (disk);

  if (dev == NULL)
    return (grub_errno != GRUB_ERR_NONE) ? grub_errno : GRUB_ACCESS_DENIED;
