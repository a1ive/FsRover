	...
	grub_uint32_t ino, struct grub_qnx4_inode* inode)
{
	...
	inode->ino = ino;
	if (grub_disk_read(data->disk, blk_num, blk_off, sizeof(inode->raw),
		&inode->raw))
		return grub_errno;
	return 0;
}
...
static int qnx4_strnlen(const char* text, int max_size)
{
	int idx;
	...
	grub_fshelp_iterate_dir_hook_t hook, void* hook_data)
{
	...
	while (fpos < i_size)
	{
		...
		if (!(de.raw.di_status & QNX4_FILE_LINK))
			...
			size = QNX4_NAME_MAX;
		size = qnx4_strnlen(de.raw.di_fname, size);
	...
	grub_fshelp_node_t node, void* data)
{
	...
	{
		info.sizeset = 1;
		info.size = grub_le_to_cpu32(node->inode.raw.di_size);
	}
	grub_free(node);
