...
lz4_decompress (char *inbuf, grub_size_t insize, grub_off_t off,
		char *outbuf, grub_size_t len, struct grub_squash_data *data)
{
  grub_size_t usize = data->blksz;
  grub_size_t copied = 0;
  char *udata;
  int res;

  if (usize < SQUASH_CHUNK_SIZE)
    usize = SQUASH_CHUNK_SIZE;
  if (insize > GRUB_INT_MAX || usize > GRUB_INT_MAX
      || off > usize || len > usize - off)
    {
      grub_error (GRUB_ERR_BAD_COMPRESSED_DATA, "invalid lz4 chunk size");
      return -1;
    }
  ...
  res = LZ4_decompress_safe (inbuf, udata, (int) insize, (int) usize);
  if (res < 0)
    {
      grub_error (GRUB_ERR_BAD_COMPRESSED_DATA, "invalid lz4 chunk");
      grub_free (udata);
      return -1;
    }

  grub_memset (outbuf, 0, len);
  if (off < (grub_size_t) res)
    {
      copied = (grub_size_t) res - off;
      if (copied > len)
	copied = len;
      grub_memcpy (outbuf, udata + off, copied);
    }
  grub_free (udata);
  return copied;
}
		 ...
		 char *outbuf, grub_size_t len, struct grub_squash_data *data)
{
  ...
  if (usize < SQUASH_CHUNK_SIZE)
    usize = SQUASH_CHUNK_SIZE;
  if (off > usize || len > usize - off)
    {
      grub_error (GRUB_ERR_BAD_COMPRESSED_DATA, "invalid zstd chunk size");
      return -1;
    }
  ...
  res = ZSTD_decompress (udata, usize, inbuf, insize);
  if (ZSTD_isError (res))
    {
      grub_error (GRUB_ERR_BAD_COMPRESSED_DATA, "invalid zstd chunk");
      grub_free (udata);
      return -1;
    }
		      ...
		      grub_fshelp_node_t node, void *data)
{
  ...
  if ((filetype & GRUB_FSHELP_TYPE_MASK) == GRUB_FSHELP_REG)
    {
      info.sizeset = 1;
      if (grub_le_to_cpu16 (node->ino.type) == SQUASH_TYPE_LONG_REGULAR)
