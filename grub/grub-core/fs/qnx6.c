...
{
	struct qnx6_blocklist_t* next; /* mount-owned allocation list */
...
struct grub_qnx6_data_t
{
	...
	qnx6_blocklist_t*       blocklists;    // all blocklists owned by this mount
};
	...
	grub_off_t pos, grub_size_t len, char* buf)
{
	return grub_fshelp_read_file(node->data->disk, node,
		...
		pos, len, buf,
		grub_qnx6_get_block,
		fs64_to_cpu(node->data, node->inode.raw.di_size),
...
qnx6_parse_superblocks(grub_qnx6_data_t* data)
{
	...
	grub_uint64_t sb2_sec = (grub_disk_native_sectors(data->disk) & ~(QNX6_SUPERBLOCK_SECTORS -
...
/**
 * Free the 'mount' structure including the pseudo-inode block lists.
 */
static void
grub_qnx6_free_data(grub_qnx6_data_t* data)
{
	qnx6_blocklist_t* fd;

	if (!data)
		return;
	while ((fd = data->blocklists) != NULL)
	{
		data->blocklists = fd->next;
		grub_free(fd);
	}
	grub_free(data);
}
...
grub_qnx6_open(struct grub_file* file, const char* name)
{
	...
	file->size = fs64_to_cpu(data, data->inode->raw.di_size);
	file->data = data;
	file->offset = 0;
	...
	void* data)
{
	...
	info.mtimeset = 1;
	info.mtime = fs32_to_cpu(ctx->data, node->inode.raw.di_mtime);
...
fail:
	if (fdiro && fdiro != &ctx.data->diropen)
		grub_free(fdiro);
	grub_qnx6_free_data(ctx.data);
	return grub_errno;
}
