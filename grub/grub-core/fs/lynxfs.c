...
struct grub_lynxfs_inode
{
	...
	grub_uint32_t i_numblocks;      /* actual number of blocks used in file */
...
indirect:
	do
	{
		...
		if (grub_disk_read(data->disk,
			...
			((fileblock >> (log_perblock * shift)) & ((1 << log_perblock) - 1)) * sizeof(
				indir),
			sizeof(indir), &indir))
			return -1;
	} while (shift--);
	...
	grub_fshelp_node_t node, void* data)
{
	...
	info.inodeset = 1;
	info.inode = (grub_uint64_t) node->ino;
	grub_free(node);
