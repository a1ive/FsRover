	   ...
	   grub_disk_read_hook_t read_hook, void *read_hook_data)
{
  ...
  if (pa >= at->end
      || (grub_size_t) (at->end - pa)
         < GRUB_NTFS_ATTRIBUTE_DATA_RUNS + sizeof (run_offset))
    return grub_error (GRUB_ERR_BAD_FS, "run offset out of range");

  attr_size = u16at (pa, GRUB_NTFS_ATTRIBUTE_LENGTH);
  run_offset = u16at (pa, GRUB_NTFS_ATTRIBUTE_DATA_RUNS);
  if (run_offset >= attr_size
      || attr_size > (grub_size_t) (at->end - pa))
    return grub_error (GRUB_ERR_BAD_FS, "run offset out of range");
