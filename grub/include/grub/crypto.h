...
/*
 * Optional progress callback for the slow password-based key derivation
 * (PBKDF2 / Argon2) that unlocks a cryptodisk.  DONE and TOTAL measure
 * the current KDF invocation; NULL (the default) disables it.  It is
 * called from whatever thread runs the KDF.
 */
extern void (*grub_cryptodisk_kdf_progress) (unsigned int done,
					     unsigned int total, void *data);
extern void *grub_cryptodisk_kdf_progress_data;
