...
struct grub_cryptodisk
{
  ...
  /*
   * Rover: some formats (BitLocker) cannot be served by the generic
   * offset-and-cipher read path.  Their plaintext sectors map to
   * different on-disk locations (relocated boot sectors), some regions
   * must read back as zeroes (in-place metadata), and the bulk cipher
   * is non-generic (AES-CBC with the Elephant diffuser).  When
   * READ_SECTORS is non-NULL, grub_cryptodisk_read() delegates the whole
   * read+decrypt to it; DEV_DATA carries the backend's private state and
   * DEV_CLOSE frees it.  Such devices are read only.
   */
  void *dev_data;
  grub_err_t (*read_sectors) (struct grub_cryptodisk *dev,
			      grub_disk_addr_t sector,
			      grub_size_t size, char *buf);
  void (*dev_close) (struct grub_cryptodisk *dev);
};
...
/* Rover: head of the unlocked-device list, for procfs key dumps. */
grub_cryptodisk_t grub_cryptodisk_list_head (void);

/*
 * Rover: unlock the cryptodisk on the source device named SOURCE_NAME
 * with the given key bytes (a passphrase or a keyfile's contents) and,
 * on success, return the resulting device name ("cryptoN") in OUT_NAME.
 */
grub_err_t
